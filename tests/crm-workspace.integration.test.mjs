  ...
  try {
    ...
    try {
      ...
      if (process.env.CRM_TEST_JSDOM_PATH) {
        ...
        const w=dom.window;
        w.document.getElementById('review-user-email').textContent='operator@example.test';
        ...
        [...w.document.querySelectorAll('#crm-saved-views button')].find(b=>b.textContent==='Duplicar vista').click();
        assert.equal(JSON.parse(w.localStorage.getItem('crm-table:operator@example.test')).views.length,2);
