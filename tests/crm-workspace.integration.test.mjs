...
test('CRM persists opportunities, assignments, stages and task lifecycle without exposing source identity', {skip:!enabled}, async () => {
  ...
  try {
    ...
    assert.equal(moved.version,2);assert.equal(moved.details.dealName,'Trato de prueba');assert.ok(moved.closedAt);
    ...
    const taskBoard=(await board(pool,{pipeline:'bariatrica'})).items[0];assert.equal(taskBoard.overdueTasks,1);assert.equal(taskBoard.nextTask.title,'Confirmar documentos');
    ...
    try {
      ...
      if (process.env.CRM_TEST_JSDOM_PATH) {
        ...
        assert.equal((await board(pool,{pipeline:'bariatrica'})).items[0].details.city,'Ciudad de prueba');
