...
export function reviewErrorCode(error) {
  if (error?.code === 'insufficient_quota' || /no (?:credits|quota)|credits remaining|credit balance.*too low|insufficient_quota/i.test(error?.message || '')) return 'ai_quota_exhausted';
...
export function createReviewer({ openai, model, pool, provider = 'openai' }) {
...
Arquitectura comprobada: Chatwoot recibe conversaciones; sell-medinet-backend entrega eventos al core clinyco_AI en Render; el core usa ${provider} (${model}) y un worker Medinet en VPS; SELL es el dashboard protegido con Google. Las sugerencias se guardan en PostgreSQL.
...
Devuelve exclusivamente JSON {"items":[{"id":"id original","verdict":"factible|requiere_validacion|no_recomendado","reason":"razón breve","proposal":"cambio sugerido","checks":"verificación necesaria antes de aplicar"}]}. Una evaluación por sugerencia, sin cambiar IDs, hasta 500 caracteres por campo. Las propuestas siempre necesitan revisión humana antes de implementación.` },
    ...
    await recordAIUsage(pool, { provider, model: completion.model || model, usage: completion.usage, purpose: 'antonia_improvement_review' });
