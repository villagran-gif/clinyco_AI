...
test('flujo Medinet Antonia por profesional o especialidad', async ({ page }) => {
  ...
  console.log('CALENDAR_STATE', JSON.stringify(calendarState, null, 2));
  ...
  for (const index of prioritizedIndices) {
    ...
    const activeTimes = activeTable.times;
    if (!activeTimes.length) continue;

    const pickedTime = pickRandomItem(activeTimes);
    const time = normalizeSpaces(pickedTime || '');
    if (!time) continue;
  ...
  console.log('SEARCH_RESULTS_MEMORY', JSON.stringify(memory, null, 2));
