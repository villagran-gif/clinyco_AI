...
export function buildPrivateLeadSnapshot(state = {}) {
  ...
  const data = {
    ...
    awaiting: clean(state.preevaluation?.awaiting)
  };
...
export function buildPrivateLeadNote(state = {}) {
  ...
  else if (data.profile_name) lines.push(`👤 Perfil WhatsApp: ${data.profile_name} (sin verificar)`);
  ...
  if (data.awaiting && FIELD_LABELS[data.awaiting]) {
    lines.push("", "⏳ SIGUIENTE DATO PENDIENTE:", `• ${FIELD_LABELS[data.awaiting]}`);
  }

  lines.push("", "Actualizada automáticamente por Antonia. Confirmar identidad y correcciones antes de decisiones clínicas.");
