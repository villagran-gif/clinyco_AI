...
parse_path = Path("chatwoot-adapter/parse.js")
parse_path.write_text(r'''// chatwoot-adapter/parse.js
//
...
client_path = Path("chatwoot-adapter/client.js")
client = client_path.read_text(encoding="utf-8")
if "export async function sendChatwootAttachment" not in client:
    client += r'''
...
procedure_block = r'''  const procedure = detectProcedure(cleanText);
  if (procedure) {
...
'''
referral_block = procedure_block + r'''
...
'''
server = replace_once(server, procedure_block, referral_block, "referral derivation")
...
start = server.index("function formatReplyForWhatsApp(text) {")
end = server.index("async function sendConversationReply(", start)
replacement = r'''function formatReplyForWhatsApp(text) {
  return String(text || "")
    .replace(/\r/g, "")
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n[ \t]+/g, "\n")
...
'''
send_new = r'''  let sentAsAudio = false;
  try {
...
'''
server = replace_once(server, send_old, send_new, "managed audio send")
...
tests_path = Path("tests/chatwoot-adapter.test.js")
tests = tests_path.read_text(encoding="utf-8")
if 'parsea contexto referral de anuncio Meta' not in tests:
    tests += r'''
