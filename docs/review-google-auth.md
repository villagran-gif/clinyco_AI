...
confirmada y comprueba su correo contra `REVIEW_ALLOWED_EMAILS`.
...
2. Registrar/invitar únicamente las cuentas aprobadas por el administrador.
   ...
   lo que permita el panel. Verificar con una cuenta invitada el ingreso con Google. El backend exige
   cuenta confirmada y correo autorizado; no usa el proveedor de creación
   como prueba del método de la sesión.
...
## Corrección para cuentas invitadas

`app_metadata.provider` representa el proveedor de creación de la cuenta, no
prueba el método de autenticación de la sesión actual. GoTrue conserva el valor
anterior al iniciar OAuth para una cuenta existente sin `invite_token`. Por eso
el backend no debe rechazar una cuenta invitada por conservar `provider=email`.
Esta corrección sustituye la comprobación de provider indicada arriba.

El panel ofrece Google y el backend verifica el token con `/user`, confirmación
y correo autorizado. Una sesión válida de Identity de ese mismo usuario, incluso
