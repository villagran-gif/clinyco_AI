...
async function searchSupportPlaceholder() {
...
async function maybeRunIdentitySearch(state, info) {
  ...
  if (ENABLE_SUPPORT_SEARCH && !state.identity.supportSearchCompleted) {
    try {
      const supportData = await searchSupportPlaceholder({
        email: state.contactDraft.c_email,
        phone: state.contactDraft.c_tel1,
        name: info.authorDisplayName || state.contactDraft.c_nombres
      });
      state.identity.supportSearchCompleted = true;
      state.identity.supportSummary = supportData ? "soporte encontrado" : "sin búsqueda real en Support";
    } catch (error) {
