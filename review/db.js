...
// ═══════════════════════════════════════════════════════════════════
//  MARKETING COSTS + LEADS + KPIs
// ═══════════════════════════════════════════════════════════════════

/** Get all marketing costs, optionally filtered by year */
...
/** Get marketing costs aggregated by month */
...
/** Delete a marketing cost entry */
export async function deleteMarketingCost(id) {
  await getPool().query(`DELETE FROM marketing_costs WHERE id = $1`, [id]);
}

/** Leads per day */
export async function leadsPerDay(year = null, limit = 90) {
  const filter = year ? `AND EXTRACT(YEAR FROM day) = ${parseInt(year)}` : '';
  const { rows } = await getPool().query(
    `SELECT day::text, new_leads, new_deals, source
     FROM leads_daily WHERE true ${filter}
     ORDER BY day DESC LIMIT $1`, [limit]
  );
  return rows;
}

/** Leads per month (aggregated) */
export async function leadsPerMonth(year = null) {
  const filter = year ? `WHERE EXTRACT(YEAR FROM day) = ${parseInt(year)}` : '';
  const { rows } = await getPool().query(
    `SELECT date_trunc('month', day)::date::text AS month,
            sum(new_leads)::int AS total_leads,
            sum(new_deals)::int AS total_deals,
            CASE WHEN sum(new_leads) > 0
              THEN round(sum(new_deals)::numeric / sum(new_leads)::numeric * 100, 1)
              ELSE 0 END AS conversion_pct
     FROM leads_daily ${filter}
     GROUP BY 1 ORDER BY 1 DESC`
  );
  return rows;
}

/** Leads from deals table (calculate from actual data if leads_daily is empty) */
export async function leadsFromDeals(year = null) {
