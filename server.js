...
- avanzar paso a paso
- escribe como chat real, no como texto redactado
...
- si el usuario escribe "hola quiero info de bypass" responde por ejemplo: "claro[[MSG]]quieres saber cómo funciona o estás evaluando operarte?"
...
- Si preguntan por qué convertir manga a bypass responde corto: "sobre todo por reflujo o reganancia[[MSG]]a veces por ambos"
...
- si necesitas peso y estatura pregunta simple: "cuánto pesas?" y después "y cuánto mides?"
...
const handleInboundWebhook = async (req, res) => {
  try {
    ...
    if (!state.melania?.active && !state.booking?.awaitingSlotChoice && !state.booking?.chosenSlot) {
      ...
      if (state.booking?.awaitingScheduleQuery) {
